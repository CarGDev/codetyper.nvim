...
function M.is_snapshot_stale(snapshot)
  ...
  local current_tick = vim.api.nvim_buf_get_var(snapshot.bufnr, "changedtick") or vim.b[snapshot.bufnr].changedtick or 0
  ...
  if current_tick ~= snapshot.changedtick then
    ...
    if snapshot.range then
      current_hash = hash_buffer_range(snapshot.bufnr, snapshot.range.start_line, snapshot.range.end_line)
    else
...
function M.create_from_event(event, generated_code, confidence, strategy)
  ...
  local is_inline = (source_bufnr == target_bufnr)
    or (event.target_path and not event.target_path:match("%.codetyper%."))
  ...
  local snapshot = M.snapshot_buffer(target_bufnr ~= -1 and target_bufnr or event.bufnr, snapshot_range)
  ...
  if event.intent_override and event.intent_override.action then
    ...
    injection_range = event.injection_range
      or (
        event.range
        and {
  ...
  local range_str = injection_range and string.format("%d-%d", injection_range.start_line, injection_range.end_line)
    or "nil"
  logger.info(
    "patch",
    string.format(
...
function M.apply(patch)
  logger.info(
    "patch",
    string.format(
      "apply() entered: id=%s strategy=%s has_range=%s",
      patch.id,
      tostring(patch.injection_strategy),
      patch.injection_range and "yes" or "no"
    )
  )
  ...
  if
    ph
    and ph.bufnr
    and vim.api.nvim_buf_is_valid(ph.bufnr)
    and not (patch.use_search_replace and patch.search_replace_blocks and #patch.search_replace_blocks > 0)
  then
    local marks_mod = require("codetyper.core.marks")
  ...
  local apply_msg = string.format(
    "apply: id=%s strategy=%s has_range=%s is_inline=%s target_bufnr=%s",
    patch.id,
...
function M.cancel_for_buffer(bufnr)
  local cancelled = 0
  for _, patch in ipairs(patches) do
    if patch.status == "pending" and (patch.target_bufnr == bufnr or patch.original_snapshot.bufnr == bufnr) then
  ...
  if patch.use_search_replace and patch.search_replace_blocks and #patch.search_replace_blocks > 0 then
    ...
    for _, block in ipairs(sorted_blocks) do
      ...
      if match then
        ...
        conflict.insert_conflict(target_bufnr, match.start_line, match.end_line, new_lines, "AI SUGGESTION")
